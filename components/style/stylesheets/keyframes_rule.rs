...
use values::computed::percentage::Percentage;
...
pub struct KeyframePercentage(pub Percentage);
...
impl KeyframePercentage {
    ...
    pub fn new(value: f32) -> KeyframePercentage {
        ...
        KeyframePercentage(Percentage(value))
    }
...
impl KeyframesAnimation {
    ...
    pub fn from_keyframes(
        ...
        if (result.steps[0].start_percentage.0).0 != 0. {
            result.steps.insert(0, KeyframesStep::new(KeyframePercentage::new(0.),
                                                      KeyframesStepValue::ComputedValues,
                                                      guard));
        }

        if (result.steps.last().unwrap().start_percentage.0).0 != 1. {
